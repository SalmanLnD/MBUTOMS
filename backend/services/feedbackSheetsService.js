...
import { buildFeedbackResponsesExportRows } from '../utils/feedbackResponsesExport.js';
import { getPublicApiBaseUrl } from '../services/appsScriptSheetsService.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const SPREADSHEET_SETTING_KEY = 'feedback_responses_spreadsheet';
const EXPORT_KEY_SETTING = 'feedback_export_api_key';
...
export const getOrCreateFeedbackExportKey = async () => {
...
export const validateFeedbackExportKey = async (key) => {
  const stored = await getExportKey();
  return Boolean(stored && key && key === stored);
};

export const getLinkedFeedbackSheet = async () => {
...
export const getFeedbackAppsScriptStatus = async () => {
  const linked = await getLinkedFeedbackSheet();
...
export const getFeedbackAppsScriptSetup = async (req) => {
  ...
  const templatePath = path.join(__dirname, '../templates/feedback-responses-sync.gs');
  ...
  return {
    ...
    script,
    steps: [
      'Create a new Google Sheet for feedback responses.',
      ...
      'Use menu TOMS Feedback → Refresh now to test.',
...
export const linkFeedbackSpreadsheetUrl = async (spreadsheetUrl) => {
...
export const unlinkFeedbackSpreadsheet = async () => {
  await AppSetting.deleteOne({ key: SPREADSHEET_SETTING_KEY });
  return { unlinked: true };
};

export const exportFeedbackResponseRows = async () => buildFeedbackResponsesExportRows();

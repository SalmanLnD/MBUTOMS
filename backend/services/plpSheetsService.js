...
import { buildPlpExportPayload } from '../controllers/plpController.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const SPREADSHEET_SETTING_KEY = 'plp_spreadsheet';
const EXPORT_KEY_SETTING = 'plp_export_api_key';
...
export const getOrCreatePlpExportKey = async () => {
...
export const validatePlpExportKey = async (key) => {
...
export const linkPlpSpreadsheet = async (spreadsheetUrl) => {
  const value = {
    mode: 'apps_script',
...
export const unlinkPlpSpreadsheet = async () => {
  await AppSetting.deleteOne({ key: SPREADSHEET_SETTING_KEY });
  return { unlinked: true };
};

export const exportPlpForSheets = async () => buildPlpExportPayload();
